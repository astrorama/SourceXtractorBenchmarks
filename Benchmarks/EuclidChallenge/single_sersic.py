...
data_file_path = '../../DEEPFIELD/FIELD%s/Single_Sersic/ssersic_%s_vis.fits' % (act_field, act_field)
rms_file_path  = '../../DEEPFIELD/FIELD%s/Single_Sersic/ssersic_%s_vis.rms.fits' % (act_field, act_field)
...
rad = FreeParameter(lambda o: o.radius, Range(lambda v, o: (.1 * v, 10*v), RangeType.EXPONENTIAL))
...
X_sersic = DependentParameter(lambda n: np.log( (n-0.3)/(5.5-n) ), sersic )
add_prior( X_sersic, -2.5, 0.7)
